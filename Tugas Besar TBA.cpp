...
int main()
{
    ...
    //inputString = "while a a > b do     c = d / e endwhile";
    ...
    while (inputString[inputPointer] != '\0') {
        ...
        if (isalpha(inputString[inputPointer])) {
            ...
            if (signature == "a") {
                ...
                scannedString[scannedPointer] = "error";
                ++scannedPointer;
                break;
            }
        }
    ...
    std::string row[] = { "S", "<condition>", "<comparison>", "<logicalOperator>", "<variable>", "<comparisonOperator>", "<boolean>", "<action>", "<operation>", "<arithmeticOperator>", "<calculatio>"};
    ...
    parseTable[0]["<comparison>"]["a"] = "<variable>";
    parseTable[1]["<comparison>"]["a"] = "<comparisonOperator>";
    parseTable[2]["<comparison>"]["a"] = "<variable>";
    parseTable[0]["<comparison>"]["b"] = "<variable>";
    parseTable[1]["<comparison>"]["b"] = "<comparisonOperator>";
    parseTable[2]["<comparison>"]["b"] = "<variable>";
    parseTable[0]["<comparison>"]["c"] = "<variable>";
    parseTable[1]["<comparison>"]["c"] = "<comparisonOperator>";
    parseTable[2]["<comparison>"]["c"] = "<variable>";
    ...
    /*while (top(stack) != "#" && top(stack) != "error")*/ for (int i = 0; i < 50; ++i) {
        ...
        for (int i = 0; i < numColumn; ++i) { // mencaritahu apakah top stack termasuk terminal
        ...
        for (int i = 0; i < numRow; ++i) { // mencaritahu apakah top stack termasuk non terminal
            if (top(stack) == row[i]) {
                topIsRow = true;
                break;
            }
        ...
        //std::cout << "topIsColumn : " << topIsColumn << "\n"; // untuk debugging apakah top stack termasuk column
        //std::cout << "topIsRow : " << topIsRow << "\n\n"; // untuk debugging apakah top stack termasuk row
        ...
        else if (topIsRow) { // jika top stack adalah non terminal
            int leftIndex = 0;
            for (int i = 4; i >= 0; --i) {
    ...
    if (top(stack) == "#")  std::cout << "input valid";
